var swaggerHttpUtils = function (utils) {
  ...
  this.call = function (callOptions) {
    ...
    */
    ...
    if(options.method === 'GET'){
      ...
      encodedParams = JSON.stringify(options.parameters);
    }
  ...
  this.constructUrl = function(options, urlParams){
    ...
    // In all cases we check if is a url containing {key} to handle things like: calendars/{calendar_id}/ ...
    // In this case the value should be in the params. If not we don't replace
    var regex = /{(.*?)}/g;
    var key, replaceRegExp;
    // If the regexp actually returns something means there is {...} in the url
    while(matches = regex.exec(url)){
      	key = matches[1];
        if(options.parameters[key]){
          replaceRegExp = new RegExp("{"+key+"}","g");
          url = url.replace(replaceRegExp, options.parameters[key]);
        }
    }
